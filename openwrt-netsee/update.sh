...
LATEST_INFO="$(github_getLatest zzxym netsee)"

# version check
LATEST_VERSION="$(jq '.tag_name' -r <<< "$LATEST_INFO" | sed 's|^v||')"
[ -n "$LATEST_VERSION" ] || exit 1
RUST_NETSEE_VERSION="$(sed -n 's|RUST_NETSEE_VERSION:=||p' "$CURDIR/Makefile")"
[ "$RUST_NETSEE_VERSION" != "$LATEST_VERSION" ] || exit 0
...
for (( i=0; i<$(jq -r '.assets | length' <<< "$LATEST_INFO"); i++ )); do
	name=$(jq -r '.assets['$i'].name | select(contains("netsee"))' <<< "$LATEST_INFO")
	[ -n "$name" ] || continue
	...
	arch="$(awk -F'-' '{ printf "%s-%s", $1, $NF }' <<< "$(sed "s|netsee-$LATEST_VERSION-||;s|\.tar\..*||" <<< "$name")")"
...
sed -i "s|\(RUST_NETSEE_VERSION:=\).*|\1$LATEST_VERSION|" "$CURDIR/Makefile"
